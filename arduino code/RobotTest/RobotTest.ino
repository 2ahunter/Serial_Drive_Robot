...
/*
 *************************************************************************************
This is to test the robot with complete external control, that is without an internal
loop timer.   The Xbee receives either requests for state updates or drive commands
**************************************************************************************
 *  using SPI, here is hardware setup:
  LSM9DS0 --------- Arduino
  CSG -------------- 17
  CSXM ------------- 9
  SDOG ------------- 12
...
unsigned long ms_delay = 2;  //for serial1 input
...
//    } 
    if (char(sByte) == '['){
      ...
      String vCmd;
      int sIndex;
      Serial1.readBytesUntil(']', sBuf, sLength);
      vCmd = String(sBuf);
      sIndex = vCmd.indexOf(',');
      v_left = vCmd.substring(0,sIndex).toInt();
      v_right = vCmd.substring(sIndex+1,sLength).toInt();
//      v_left = Serial1.parseInt();
//      v_right = Serial1.parseInt();
//      Serial1.print("ACK: ");
//      Serial1.print(v_left);
//      Serial1.print(",");
//      Serial1.println(v_right);
...
void updateRobot() {
  ...
  robot.theta = dTheta*180/PI; //update the calculated heading from odometry
...
//    Serial1.print(" ");
    Serial1.print(odometry.theta_dot,4);
    Serial1.print(" ]\n");
    Serial1.flush();
}
